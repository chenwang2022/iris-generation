...
class iris_verification(nn.Module):
    def __init__(self):
        super(iris_verification, self).__init__()
...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, irisnet, device):
    ...
    for idx in pbar:
        ...
        if g_regularize:
            ...
            noise = make_noises(path_batch_size, args.latent, device, one_id=False)
        ...
        if get_rank() == 0:
            pbar.set_description(
                (
                    ...
                    f"inter-class: {id_loss_f_val:.4f}; intra-class: {id_loss_t_val:.4f}; "
                )
            )
                ...
                wandb.log(
                    {
                        ...
                        "r1": r1_val,
                        "inter-class": id_loss_f_val,
                        "intra-class": id_loss_t_val,
                    }
                )
            ...
            if i % 10000 == 0:
                ...
                with torch.no_grad():
                    ...
                    utils.save_image(
                        sample,
                        f"sample/{str(i).zfill(7)}_inter-class.png",
                    ...
                    utils.save_image(
                        sample,
                        f"sample/{str(i).zfill(7)}_intra-class.png",
                        nrow=int(args.n_sample ** 0.5),
                        normalize=True,
                        range=(-1, 1),
                    )
...
if __name__ == "__main__":
    ...
    irisnet = iris_verification().to(device)
    irisnet.eval()
